...
def dichotomy_step_ralgo(main_circle_radius: float, circles: list[Circle]) -> tuple[float, list[Circle]]:
    ...
    while step_size >= 0.01:
        while True:
            y = ralgb5(np.copy(x), 3, step_size, 1, 1e-6,
                       1e-7, 3000, circles_radiuses)
            if y[-1] >= x[-1]:
                break
            x = np.copy(y)
...
def main(radiuses: list[float], r_algo_optimization=True) -> tuple[float, list[Circle]]:
    ...
    if r_algo_optimization:
        main_circle_radius, circles = dichotomy_step_ralgo(
            main_circle_radius, circles)
    ...
    draw_plot(main_circle_radius, circles,
              'packed_circles', f'R={main_circle_radius}')


number_of_circles = int(input())
radiuses = [float(input()) for _ in range(number_of_circles)]
